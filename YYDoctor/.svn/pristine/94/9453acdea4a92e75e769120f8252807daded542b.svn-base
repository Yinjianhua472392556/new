...
- (void)addNewGroup {
    ...
    NSDictionary *params = @{@"userId":loginInfo.userId,
                             ...
                             @"orderNum":@(orderNum)};
    [PatientHttpTool addNewPatientGroup:params completion:^(id result, NSError *error) {
        if (!result) {
            [HUDHelper showError:error.description];
            return;
        }
...
- (void)tableView:(UITableView *)tableView moveRowAtIndexPath:(NSIndexPath *)sourceIndexPath toIndexPath:(NSIndexPath *)destinationIndexPath {
    if (sourceIndexPath != destinationIndexPath) {
        ...
        [PatientHttpTool modifyGroupSort:@{@"param":params} completion:^(id result, NSError *error) {
            if (!result) {
                [HUDHelper showError:error.description];
                return ;
            }
