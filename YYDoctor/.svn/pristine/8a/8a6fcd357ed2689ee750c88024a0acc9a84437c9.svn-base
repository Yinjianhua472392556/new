...
- (void)setupDataWithType:(NSInteger )type {
    LoginInfo *loginInfo = [LoginInfo currentLoginInfo];
    NSString *path = @"CloudUser/ws/rest/consultRecord/userList?";
...
/**    if(self.segmentControl.selectedSegmentIndex == 0) {//付费列表
        ...
        if (model) { //有缓存
            ...
            [path appendFormat:@"JavaBridgeTemplate621/api/app/users/%@",conversation.chatter];
...
- (void)viewDidLoad {
    ...
    [self setupDataWithType:2]; //1免费，2付费
