...
- (void)requestServiceState {
    ...
    NSString *path = @"CloudDoctor/ws/rest/alertSet/select";
    [YYAPIRequest GET:path params:@{@"docId":loginInfo.doctorInfo.doctorId} completion:^(id response, NSError *error) {
        if (response) {
...
//请求咨询时段
- (void)requestTime {
    NSString *path = @"CloudDoctor/ws/rest/timeSlot/select";
    NSNumber *doctorId = [LoginInfo currentLoginInfo].doctorInfo.doctorId;
    [YYAPIRequest GET:path params:@{@"docId":doctorId} completion:^(id response, NSError *error) {
...
- (void)modifyService:(NSString *)service state:(BOOL)isOn {
    NSString *path = @"CloudDoctor/ws/rest/alertSet/";
    LoginInfo *loginInfo = [LoginInfo currentLoginInfo];
...
- (BOOL)timeValidate {
    ...
    for (SettingsTimeInfo *info in serveTimes) {
        ...
        if (resultA == NSOrderedSame && resultB == NSOrderedAscending) {
            [HUDHelper showError:@"时间段有重叠哟"];
            return NO;
        }
...
- (IBAction)onTimeSelected:(id)sender {
    ...
    info.doctorId = [LoginInfo currentLoginInfo].doctorInfo.doctorId;
...
- (void)settingsTimeSaveClick:(UIButton *)sender {
    ...
    NSString *path = @"CloudDoctor/ws/rest/timeSlot/set";
    [YYAPIRequest POST:path params:@{@"param":json} completion:^(id response, NSError *error) {
        if (response) {
            NSLog(@"%@",response);
        }else {
            NSLog(@"%@",error);
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (indexPath.section == 0) {
        ...
        if (![items[indexPath.row] isEqual:[NSNull null]]) {
